...
class PanelPlot(object):
    ...
    def flatten(self, attachment_points):
        ...
        cut_allowances = {
            ...
            "hybrid_top": self.config.allowance_design,
            "hybrid_bottom": self.config.allowance_entry_open,
        }
        ...
        cut_types = {
            ...
            "hybrid_top": self.config.cut_3d,
            "hybrid_bottom": self.config.cut_entry,
        }
        ...
        if (self.panel.cut_front["type"] != "cut_3d") and (self.panel.cut_front["type"] != "hybrid_top"):
            dist = np.linspace(
                shape_3d_amount_front[0],
                shape_3d_amount_front[-1],
            ...
            {
                print(shape_3d_amount_front)
            }

        if (self.panel.cut_back["type"] != "cut_3d") and (self.panel.cut_back["type"] != "hybrid_top"):
            dist = np.linspace(
                shape_3d_amount_back[0],
                shape_3d_amount_back[-1],
            ...
            {
                print(shape_3d_amount_back)
            }

        # For cut_3d type, use outer_orig to avoid geometry mismatch from .check()
        outer_left = self.outer_orig[0] if self.panel.cut_front["type"] == "cut_3d" or self.panel.cut_back["type"] == "cut_3d" or self.panel.cut_front["type"] == "hybrid_top" or self.panel.cut_back["type"] == "hybrid_top" else self.outer[0]
        outer_right = self.outer_orig[1] if self.panel.cut_front["type"] == "cut_3d" or self.panel.cut_back["type"] == "cut_3d" or self.panel.cut_front["type"] == "hybrid_top" or self.panel.cut_back["type"] == "hybrid_top" else self.outer[1]
