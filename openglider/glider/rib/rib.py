...
class SingleSkinRib(Rib):
    ...
    def get_hull(self, glider=None):
        ...
        te_end = self.single_skin_par.get("te_end", 1.0)

        # Chercher le cut singleskin dans les cellules adjacentes
        # pour faire démarrer le premier bow depuis ce point
        # Comparaison par identité OU par nom (robuste avec nervures miroir)
        singleskin_cut = None
        rib_name = getattr(self, "name", None)
        # Résoudre la nervure miroir si nécessaire
        rib_ref = self
        if hasattr(self, "mirrored_rib") and self.mirrored_rib:
            rib_ref = self.mirrored_rib
            rib_name = getattr(rib_ref, "name", rib_name)
        for cell in glider.cells:
            rib1_match = (cell.rib1 is self or cell.rib1 is rib_ref or
                          (rib_name and getattr(cell.rib1, "name", None) == rib_name))
            rib2_match = (cell.rib2 is self or cell.rib2 is rib_ref or
        ...
        # Construire pos : att_pts + te_end
        att_pos = [att.rib_pos for att in attach_pts if att.rib_pos < te_end]
        if singleskin_cut is not None and att_pos:
            # Remplacer le premier att_pt par le cut singleskin
            att_pos_sorted = sorted(att_pos)
            att_pos_rest = [p for p in att_pos_sorted if p > singleskin_cut]
            pos = list(set([singleskin_cut] + att_pos_rest + [te_end]))
            # double_first : ne s'applique que si au moins 2 att_pts sont
            # dans la zone des bows (après singleskin_cut)
            # Si moins de 2 att_pts après le cut → double_first désactivé
            effective_double_first = (
                self.single_skin_par["double_first"] and
                len(att_pos_rest) >= 2
            )
        else:
            pos = list(set(att_pos + [te_end]))
            effective_double_first = self.single_skin_par["double_first"]
        ...
        if len(pos) > 1:
            ...
            for k, sp in enumerate(span_list):
                if effective_double_first and k == 0:
            ...
            for i, sp in enumerate(span_list):
                ...
                if effective_double_first and i == 0:
                    continue
                ...
                # height peut être une liste (une valeur par bow) ou un float (rétrocompatibilité)
                height_param = self.single_skin_par["height"]
                if isinstance(height_param, (list, tuple)):
                    h = height_param[min(i, len(height_param) - 1)]
                else:
                    h = height_param
                height *= h
        ...
        # --- Coupure extrados à te_end ---
        # Coupure du profil à te_end si < 1.0
        # Les bows sont déjà calculés jusqu'à te_end via pos.
        # Il reste à tronquer le profil : garder extrados(te_end→LE) + intrados(LE→te_end)
        if te_end < 1.0 and len(span_list) > 0:
            last_bow_start = span_list[-1][0]  # début du dernier bow = att_pt + att_dist/2
            profile.insert_point(-te_end)          # extrados à x=te_end
            profile.insert_point(last_bow_start)   # intrados au début du dernier bow
            i_ext = int(round(profile(-te_end)))
            i_int = int(round(profile(last_bow_start)))
            p_ext = np.array(profile.data[i_ext])  # point extrados à te_end
            p_int = np.array(profile.data[i_int])  # point intrados début dernier bow

            # Courbe de fermeture : Bézier quadratique de p_int vers p_ext
            # camber > 0 : bombe vers l'extérieur (vers le bas)
            camber = self.single_skin_par.get("camber", 0.0)
            if camber != 0.0:
                mid = (p_int + p_ext) / 2
                seg = p_ext - p_int
                seg_len = np.linalg.norm(seg)
                # Perpendiculaire pointant vers l'extérieur (rotation 90° horaire)
                perp = np.array([-seg[1], seg[0]])
                perp = perp / np.linalg.norm(perp)
                p_ctrl = mid + perp * camber * seg_len
                closure_pts = []
                # endpoint=False pour t=0 : évite le doublon avec le dernier point de l'intrados
                # endpoint=True pour t=1 : inclure p_ext comme point final
                for t in np.linspace(0, 1, 21)[1:]:
                    pt = (1-t)**2 * p_int + 2*(1-t)*t * p_ctrl + t**2 * p_ext
                    closure_pts.append(pt)
            else:
                closure_pts = [p_ext]

            new_data = list(profile.data[i_ext:i_int + 1]) + closure_pts
            profile.data = np.array(new_data)
        return profile
